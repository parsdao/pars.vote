...
type ContractsBase = {
  ...
  // Cyrus ecosystem contracts (optional - for Cyrus governance)
  cyrus?: {
    token: Address;           // CYRUS governance token
    dao: Address;             // CyrusDAO
    pars: Address;            // PARS emissions token
    spars: Address;           // sPARS rebasing staked token
    gaugeController: Address; // CyrusGaugeController
    rewardsGauge: Address;    // CyrusRewardsGauge
    mockLP?: Address;         // Mock LP token (dev only)
    mockUSDT?: Address;       // Mock USDT (dev only)
  };
};
